...
/**
 * This class holds a Format, and has methods for reading and writing data
 * against it.
 */
public class Formatter {
	...
	public void write(Vector v, PrintStream out) throws OutputFormatException {
		FormatOutputList vp = new VectorAndPointer(v);
		this.format.write(vp, out);
	}

	public void write(int i, PrintStream out) throws OutputFormatException {
		write(new Integer(i), out);
	}

	public void write(long l, PrintStream out) throws OutputFormatException {
		write(new Long(l), out);
	}

	public void write(float f, PrintStream out) throws OutputFormatException {
		write(new Float(f), out);
	}

	public void write(double d, PrintStream out) throws OutputFormatException {
	...
	public void read(Vector v, Hashtable ht, DataInputStream in) throws InputFormatException {
		FormatInputList vp = new StringsHashtableAndPointer(v, ht);
		InputStreamAndBuffer inb = new InputStreamAndBuffer(in);
		this.format.read(vp, inb, this.format_map);
	}

	public void read(String[] s, Hashtable ht, DataInputStream in) throws InputFormatException {
		Vector v = new Vector();
	...
	public Object read(DataInputStream in) throws InputFormatException {
		Vector v = new Vector();
		read(v, in);
		return v.elementAt(0);
	}

	public boolean eof(DataInputStream in) throws IOException {
